...
export class MouseController {
  ...
  handleTouchScroll(e, deltaY) {
    ...
    const isReading = app.site?.pageState === PAGE_STATE.READING;
    const navUp = isReading ? isScrollUp : !isScrollUp;
    const cmd = navUp ? 'doArrowUp' : 'doArrowDown';
    const navDirection = navUp ? 'up' : 'down';
    this._dispatchWheelNav(cmd, steps, navDirection, now);
    return true;
  }
}
