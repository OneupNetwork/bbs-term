...
test('TouchController touch scroll sends up/down or j/k when mouse report is off, and SGR wheel report when mouse report is on', () => {
  ...
  // 1. Without mouse report & non-reading site -> drag down (200 -> 230) moves cursor down (doArrowDown)
  listeners.pointerdown({
    pointerType: 'touch',
    pointerId: 1,
    clientX: 200,
  ...
  listeners.pointermove({
    ...
    pointerId: 1,
    clientX: 200,
    clientY: 230, // dragged down 30px (>= chh 24px) -> cursor follows finger down
    preventDefault() {},
  });
  listeners.pointerup({
    pointerType: 'touch',
  ...
  assert.deepEqual(navCmds, ['doArrowDown'], 'Touch drag down without mouse report in list/menu sends doArrowDown');
  navCmds.length = 0;
